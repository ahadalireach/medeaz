...
class GeminiService {
  ...
  async discoverModel() {
    for (const modelId of MODEL_CANDIDATES) {
      try {
        ...
        console.log(`Gemini Service: Auto-discovered working model: ${modelId}`);
        return true;
      } catch (err) {
  ...
  async chat(message, conversationHistory = []) {
    try {
      ...
      const attemptWithModel = async (currentModelId) => {
        const modelObj = this.genAI.getGenerativeModel({ model: currentModelId });
        const chat = modelObj.startChat({
          history: [
            { role: 'user', parts: [{ text: CHAT_SYSTEM_PROMPT }] },
            { role: 'model', parts: [{ text: 'Understood. I am ready to assist patients with general health guidance.' }] },
            ...conversationHistory.filter(h => h.content?.trim()).map(msg => ({
      ...
      try {
        ...
        const isQuotaIssue = error.message?.includes('429') || error.message?.includes('quota');
        const isNotFound = error.message?.includes('not found') || error.message?.includes('404');

        if (isQuotaIssue || isNotFound) {
          console.log(`Model ${this.activeModelId} failed (${isQuotaIssue ? 'Quota' : '404'}). Searching for alternative...`);
          
          // Filter out the one that just failed
          const alternatives = MODEL_CANDIDATES.filter(m => m !== this.activeModelId);
      ...
      throw new Error(error.message || 'AI service currently experiencing high load. Please try again soon.');
    }
  }

  async parsePrescription(transcription) {
    try {
      if (!this.apiKey || !this.genAI) return this.parseWithRules(transcription);
      const result = await this.model.generateContent(`Parse this prescription: "${transcription}"`);
      const response = await result.response;
  ...
  parseWithRules(transcription) {
    return { diagnosis: 'General consultation', medicines: [], notes: transcription, parsedBy: 'fallback' };
  }
...
module.exports.chatWithGemini = (message, history) => geminiServiceInstance.chat(message, history);
