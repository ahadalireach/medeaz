...
import {
  ...
  CheckCircle2, X, RefreshCw, Save, Loader
} from "lucide-react";
...
export default function SchedulePage() {
  ...
  const getDayLabel = (dayName: string) => {
    if (locale === 'ur') {
      ...
      return mapping[dayName.toLowerCase()] || dayName;
    }
    return dayName;
  };
  ...
  const timeSlots = useMemo(() => {
    if (shift === 'day') {
      ...
      // Night Shift: 20:00 to 08:00 (next day)
      return Array.from({ length: 48 }, (_, i) => {
        const totalHours = Math.floor(i / 4) + 20;
        const hour = totalHours >= 24 ? totalHours - 24 : totalHours;
        if (totalHours >= 32) return null;
        const min = (i % 4) * 15;
        return `${hour.toString().padStart(2, '0')}:${min.toString().padStart(2, '0')}`;
      }).filter(Boolean) as string[];
    }
  ...
  const isSlotOffForClinic = (dayName: string, time: string) => {
    ...
    let openTime = dayHours.open || "09:00";
    let closeTime = dayHours.close || "17:00";

    const [openH] = openTime.split(':').map(Number);
    const [closeH, closeM] = closeTime.split(':').map(Number);
    if (closeH < openH && closeH <= 12) {
      closeTime = `${(closeH + 12).toString().padStart(2, '0')}:${closeM.toString().padStart(2, '0')}`;
    }
  ...
  const handleCreateFollowUp = async (days: number) => {
    ...
    try {
      ...
      toast.success(locale === 'ur' ? `فالو اپ اپائنٹمنٹ کامیابی سے بن گئی!` : `Follow-up appointment created for ${format(futureDate, 'MMM d, yyyy')}`);
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'completed': return 'bg-primary border-primary/80 text-white';
      case 'in-progress': return 'bg-purple-600 border-purple-700 text-white';
      case 'confirmed': return 'bg-blue-500 border-blue-600 text-white';
      case 'cancelled': return 'bg-red-500 border-red-600 text-white';
      default: return 'bg-amber-500 border-amber-600 text-white'; // pending
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 w-full">
      <div className="space-y-6 animate-in fade-in duration-500 min-w-0 w-full">
        <PageHeader title="Your schedule" />
        {/* Header / Week Navigator */}
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white dark:bg-[#18181b] p-4 rounded-2xl shadow-sm border border-black/5 dark:border-white/10">
          <div className="flex items-center justify-between gap-4 w-full lg:w-auto">
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-primary text-white rounded-xl font-bold hover:bg-primary-hover transition-all disabled:opacity-50 text-xs sm:text-sm shadow-md"
            >
              {saving ? (
                <>
                  <Loader className="h-3 w-3 sm:h-4 sm:w-4 animate-spin" /> {locale === 'ur' ? 'محفوظ ہو رہا ہے...' : 'Saving...'}
                </>
              ) : (
                <>
                  <Save className="h-3 w-3 sm:h-4 sm:w-4" /> {locale === 'ur' ? 'محفوظ کریں' : (t('doctor.saveSchedule') || 'Save')}
                </>
              )}
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 bg-surface p-1.5 rounded-xl border border-black/5 dark:border-white/5 mx-auto lg:mx-0">
            <button
              onClick={() => setShift('day')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${shift === 'day' ? 'bg-primary text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
            >
              ☀️ {locale === 'ur' ? 'دن کی شفٹ' : 'Day Shift'}
            </button>
            <button
              onClick={() => setShift('night')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${shift === 'night' ? 'bg-[#3b82f6] text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
            >
              🌙 {locale === 'ur' ? 'رات کی شفٹ' : 'Night Shift'}
            </button>
          </div>

          <div className="flex items-center justify-between sm:justify-start gap-2 bg-surface p-1.5 rounded-xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-primary/5 p-5 rounded-2xl shadow-sm border border-primary/20 relative overflow-hidden">
            <div className="text-sm font-semibold text-primary">
              {locale === 'ur' ? 'ڈاکٹر کا استعمال' : 'Doctor Utilization'}
            </div>
        ...
        {/* Calendar Grid */}
        <div className="bg-white dark:bg-[#18181b] rounded-2xl border border-black/5 dark:border-white/10 shadow-sm overflow-hidden flex flex-col w-full min-w-0" dir="ltr">
          <div className="flex overflow-x-auto pb-4">
            ...
            <div className="flex-none w-[70px] sm:w-[80px] shrink-0 flex flex-col border-r border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/5">
              <div className="h-40 border-b border-gray-100 dark:border-white/5 p-3"></div>
              {timeSlots.map(time => (
                <div key={time} className="h-12 border-b border-gray-100 dark:border-white/5 text-[10px] sm:text-xs font-semibold text-gray-500 flex items-center justify-center bg-gray-50/30 dark:bg-[#18181b]/30">
            ...
            {daysArr.map(dayName => {
              ...
              return (
                <div key={dateStr} className="flex-1 min-w-[140px] sm:min-w-[180px] flex flex-col border-r border-gray-100 dark:border-white/5 relative group">
                  <div className="h-40 border-b border-gray-100 dark:border-white/5 p-3 bg-white dark:bg-[#18181b] sticky top-0 z-10">
                    <div className="font-bold text-gray-900 dark:text-white capitalize">{getDayLabel(dayName)}</div>
                    ...
                    <div className="mt-2 text-[10px] sm:text-xs bg-primary/10 text-primary font-bold px-2 py-1 rounded-lg w-fit">
                      {locale === 'ur' ? (
                        <>{stats.appointments} اپائنٹمنٹس • {stats.patients} مریض</>
                      ) : (
                    ...
                    <div className="text-[10px] mt-1 font-semibold text-gray-500">
                      {locale === 'ur' ? (
                        <>{stats.availableSlotsRemaining} سلاٹس دستیاب</>
                      ) : (
                  ...
                  <div className="relative">
                    {timeSlots.map(time => {
                      ...
                      return (
                        <div key={time}
                          onClick={() => (isSlotOffForClinic(dayName, time) ? toast.error(locale === 'ur' ? "اس سلاٹ وقت کے دوران کلینک بند ہوتا ہے۔" : "Clinic is closed during this slot time.") : toggleSlotAvailability(dayName, time, isAvailable))}
                          className={`h-12 border-b border-gray-100 dark:border-white/5 transition-colors ${isSlotOffForClinic(dayName, time) ? 'bg-slate-100/50 dark:bg-slate-800/10 cursor-not-allowed select-none opacity-50' : isAvailable ? 'bg-primary hover:bg-primary-hover text-white cursor-pointer' : 'hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer'}`}
                        >
                          {isSlotOffForClinic(dayName, time) ? (
                            <div className="h-full w-full flex flex-col items-center justify-center p-0.5 leading-none">
                              <span className="text-[8px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-wider">
                                {locale === 'ur' ? 'بند' : 'Closed'}
                              </span>
                              {data?.data?.clinic?.workingHours?.[dayName] && (() => {
                                const dayHours = data.data.clinic.workingHours[dayName];
                                let openTime = dayHours.open || "09:00";
                                let closeTime = dayHours.close || "17:00";
                                const [openH] = openTime.split(':').map(Number);
                                const [closeH, closeM] = closeTime.split(':').map(Number);
                                ...
                                return (
                                  <span className="text-[7px] text-gray-400 dark:text-gray-500 font-semibold scale-90 mt-0.5 whitespace-nowrap">
                                    {formatTime12h(openTime)} - {formatTime12h(closeTime)}
                                  </span>
                                );
                              })()}
                            </div>
                          ) : null} {isAvailable && !isCovered && !isSlotOffForClinic(dayName, time) && (
                    ...
                    {dayAppointments.map((app: any) => {
                      const topIndex = timeSlots.indexOf(app.startTime);
                          ...
                          <div className="flex flex-col h-full justify-between min-w-0">
                            <div className="flex items-center justify-between gap-1 w-full min-w-0">
                              <div className="flex items-center gap-1.5 min-w-0">
                                {app.patientPhoto ? (
                                  <img src={app.patientPhoto} alt="" className="w-5 h-5 rounded-full object-cover shrink-0" />
                                ) : (
                                  <div className="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center shrink-0">
                                    <User size={12} className="text-white" />
                                  </div>
                                )}
                                <div className="font-extrabold text-sm sm:text-base text-white truncate">{app.patientName || "Patient"}</div>
                              </div>
                              <span className="shrink-0 text-[8px] sm:text-[9px] font-black uppercase tracking-wider bg-black/25 text-white px-1.5 py-0.5 rounded-full">
          ...
          <div className="bg-white dark:bg-[#18181b] rounded-2xl max-w-sm w-full p-6 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-lg truncate">{selectedAppointment.patientName || (locale === 'ur' ? 'مریض' : 'Patient')}</h3>
                <p className="text-xs text-text-secondary uppercase tracking-widest font-bold truncate">
                  {selectedAppointment.appointmentType === 'follow-up' ? (locale === 'ur' ? 'فالو اپ' : 'Follow-up') : (locale === 'ur' ? 'مشاورت' : 'Consultation')}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
              <div className="flex justify-between items-center bg-surface p-3 rounded-xl border">
                <div className="text-sm font-semibold flex items-center gap-2"><Clock size={16} /> {locale === 'ur' ? 'وقت' : 'Time'}</div>
                <div className="text-sm font-medium">{selectedAppointment.startTime} - {selectedAppointment.endTime}</div>
              </div>
              <div className="flex justify-between items-center bg-surface p-3 rounded-xl border">
                <div className="text-sm font-semibold flex items-center gap-2"><CheckCircle2 size={16} /> {locale === 'ur' ? 'حالت' : 'Status'}</div>
            ...
            <div className="border-t border-black/5 dark:border-white/10 pt-4">
              <h4 className="text-xs font-black text-text-secondary mb-3 uppercase tracking-widest">{locale === 'ur' ? 'فوری کارروائیاں' : 'Quick Actions'}</h4>
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <RefreshCw size={14} /> {locale === 'ur' ? 'فالو اپ (7 دن)' : 'Follow-up (7d)'}
                </button>
                <button
                  onClick={() => handleCreateFollowUp(14)}
                  className="flex items-center justify-center gap-2 bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 p-2 text-xs font-bold rounded-xl transition-colors"
                >
                  <RefreshCw size={14} /> {locale === 'ur' ? 'فالو اپ (14 دن)' : 'Follow-up (14d)'}
                </button>
              </div>
            </div>
          </div>
