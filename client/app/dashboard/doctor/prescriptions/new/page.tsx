  ...
  return (
    <div className="pb-12 animate-in space-y-5">
      ...
      <div className="flex items-center gap-2 md:gap-4 mb-8 px-2 md:px-0">
        <div className={`flex items-center gap-1 md:gap-2 ${step === "select" ? "text-primary" : step === "record" || step === "review" ? "text-green-600" : "text-text-secondary "}`}>
          <div className={`h-8 w-8 md:h-10 md:w-10 rounded-full flex items-center justify-center font-bold text-sm md:text-base shrink-0 ${step === "select" ? "bg-primary text-white" : step === "record" || step === "review" ? "bg-green-500 text-white" : "bg-gray-200  text-text-secondary "}`}>
            1
          </div>
        ...
        <div className={`flex items-center gap-1 md:gap-2 ${step === "record" ? "text-primary" : step === "review" ? "text-green-600" : "text-text-secondary "}`}>
          <div className={`h-8 w-8 md:h-10 md:w-10 rounded-full flex items-center justify-center font-bold text-sm md:text-base shrink-0 ${step === "record" ? "bg-primary text-white" : step === "review" ? "bg-green-500 text-white" : "bg-gray-200  text-text-secondary "}`}>
            2
          </div>
        ...
        <div className={`flex items-center gap-1 md:gap-2 ${step === "review" ? "text-primary" : "text-text-secondary "}`}>
          <div className={`h-8 w-8 md:h-10 md:w-10 rounded-full flex items-center justify-center font-bold text-sm md:text-base shrink-0 ${step === "review" ? "bg-primary text-white" : "bg-gray-200  text-text-secondary "}`}>
            3
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-100 overflow-y-auto p-2 mb-6">
                {patients.length > 0 ? (
                  patients.map((patient: any) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-text-primary  truncate">{patient.name}</p>
                        <p className="text-xs font-semibold text-text-secondary  truncate">{patient.email}</p>
                      </div>
                    </button>
                  ))
                ) : (
                  <div className="col-span-full py-10 text-center bg-gray-50 /50 rounded-2xl border-2 border-dashed border-black/6 ">
                    ...
                    <p className="text-xs text-text-muted mt-1">{t("doctor.appointments.adjustFilters")}</p>
                  </div>
                )}
              </div>

              <div className="text-center py-4">
                <p className="text-text-secondary dark:text-text-secondary text-xs md:text-sm mb-4">{t("doctor.patients.notInList")}</p>
                <Link href="/dashboard/doctor/patients/new" className="inline-flex items-center gap-2 px-8 py-3 bg-gray-100 text-primary rounded-xl text-base font-bold hover:bg-gray-200  transition-colors border border-black/6  shadow-sm">
      ...
      {step === "review" && (
        <div className="space-y-6">
          <div className="bg-white  rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
            <label className="block text-sm font-semibold text-gray-600  mb-3">{t("doctor.diagnosis")}</label>
            <textarea
              ...
              {prescriptionData.medicines.map((med, index) => (
                <div key={index} className="p-5 bg-surface/30 rounded-xl border border-gray-200 dark:border-gray-700">
                  <div className="flex items-start justify-between mb-4">
                    <span className="font-semibold text-gray-600 ">{t("prescription.medicine")} {index + 1}</span>
          ...
          <div className="bg-white  rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5 flex-1 w-full">
                <label className="text-xs font-bold uppercase tracking-widest text-text-secondary  mb-1">{t("doctor.prescriptions.consultationFee")} ({t("common.pkr")})</label>
                <input
                  type="number"
                  min="0"
                  step="1"
              ...
              <div className="space-y-1.5 flex-1 w-full">
                <label className="text-xs font-bold uppercase tracking-widest text-text-secondary  mb-1">{t("doctor.prescriptions.medicineCost")}</label>
                <input
                  type="number"
                  min="0"
                  step="1"
          ...
          <div className="bg-white  rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
            <label className="block text-sm font-semibold text-gray-600  mb-3">{t("doctor.notes")}</label>
            <textarea
          ...
          {prescriptionData.rawTranscript && (
            <div className="bg-surface/50 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
              <h4 className="text-sm font-semibold text-gray-600  mb-3">{t("doctor.prescriptions.originalTranscript")}</h4>
              <p className="text-gray-900 dark:text-gray-100">{prescriptionData.rawTranscript}</p>
            </div>
          )}

          <div className="flex items-center justify-end gap-4">
            <button onClick={() => setStep("record")} className="px-8 py-4 border border-black/6 dark:border-gray-700 rounded-xl font-semibold hover:border-primary hover:text-primary transition-all">
              {t("common.back")}
            </button>
            <button onClick={handleSubmit} disabled={creating} className="px-8 py-4 bg-primary text-white rounded-2xl font-bold hover:bg-primary-hover transition-all disabled:opacity-50 flex items-center gap-2">
              {creating ? (
                <>
