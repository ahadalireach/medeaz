    ...
    return (
        <div className="flex flex-col lg:flex-row gap-8 max-w-[1550px] mx-auto pb-20 px-4">
            ...
            <aside className="w-full lg:w-80 flex-shrink-0 space-y-6">
                <div className="bg-white dark:bg-zinc-900 rounded-[2rem] p-6 shadow-lens border border-black/5 dark:border-white/5 sticky top-24">
                    ...
                    <div className="space-y-6">
                        {/* Search */}
                        <div>
                            <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block">{t('common.search')}</label>
                            <div className="relative">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                                <input
                                    type="text"
                                    placeholder={t('patient.findDoctors.searchPlaceholder')}
                                    className="w-full pl-11 pr-4 h-12 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/50 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-4 focus:ring-primary/5 focus:border-primary-hover focus:outline-none transition-all shadow-sm"
                        ...
                        {/* Specialization */}
                        <div>
                            <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block">{t('doctor.profile.specialization')}</label>
                            <select
                        ...
                        {/* City */}
                        <div>
                            <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block">{t('form.city')}</label>
                            <select
                                className="lens-input w-full h-12 text-sm appearance-none bg-no-repeat"
                                style={{ backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%239ca3af' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`, backgroundPosition: `right 0.5rem center`, backgroundSize: `1.5em 1.5em` }}
                                value={filters.city}
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t('doctor.profile.experience')}</label>
                        ...
                        <div className="flex flex-col gap-3 pt-4 border-t border-black/5 dark:border-white/5">
                            ...
                            <button
                                ...
                                className="w-full text-xs font-bold text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors uppercase tracking-widest"
                            >
                                {t('common.clearAll')}
                            </button>
                        </div>
            ...
            <main className="flex-1 space-y-6">
                <div className="flex items-end justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">{t('patient.findDoctors.title')}</h1>
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                        {[1, 2, 3, 4].map(n => (
                            <div key={n} className="h-64 rounded-[2rem] bg-gray-100 dark:bg-zinc-800 animate-pulse" />
                        ))}
                    </div>
                ) : filteredDoctors.length === 0 ? (
                    <div className="py-20 text-center bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-black/5 dark:border-white/5 shadow-lens">
                        <div className="h-20 w-20 mx-auto bg-gray-50 dark:bg-zinc-800 rounded-full flex items-center justify-center mb-6">
                            <Search className="h-10 w-10 text-gray-400" />
                        </div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t('patient.findDoctors.noDoctors')}</h3>
                        <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium max-w-sm mx-auto">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 gap-8">
                        {filteredDoctors.map((doc: any) => (
                            <div key={doc._id} className="group relative bg-white dark:bg-zinc-900 rounded-[2rem] p-6 border border-black/5 dark:border-white/5 shadow-lens hover:border-primary/20 transition-all duration-300">

                                <div className="flex gap-5">
                                    <div className="relative">
                                        <div className="w-24 h-24 rounded-2xl overflow-hidden bg-slate-50 dark:bg-zinc-800 border-2 border-slate-100 dark:border-zinc-700 group-hover:border-primary/30 transition-all flex items-center justify-center">
                                            {doc.userId?.photo ? (
                                    ...
                                    <div className="flex-1 min-w-0 pt-1">
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-white truncate">{t('patient.bookAppointmentPage.doctorPrefix')} {doc.userId?.name}</h3>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 p-4 rounded-2xl bg-gray-50 dark:bg-zinc-800/50">
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t('doctor.profile.specialization')}</p>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{doc.specialization}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t('doctor.profile.experience')}</p>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white">{t('patient.findDoctors.yearsCount', { n: doc.experience || 0 })}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t('doctor.profile.consultationFee')}</p>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white">{t('common.pkr')} {doc.consultationFee || "TBD"}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t('patient.findDoctors.clinic')}</p>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white truncate max-w-[150px]">{(doc.clinicId?.name && !doc.clinicId.name.includes("#")) ? doc.clinicId.name : (doc.clinicId?.name?.includes("#") ? "MedEaz Health Clinic" : "Private Practice")}</p>
                                    </div>
                                </div>
