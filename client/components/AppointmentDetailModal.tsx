        ...
        >
            {loading ? (
                ...
                <div className="space-y-8 pb-4">
                    {/* Header Info */}
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-4xl border border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-4">
                            <div className="h-14 w-14 rounded-2xl bg-primary/10 flex items-center justify-center">
                                <Calendar className="h-7 w-7 text-primary" />
                            </div>
                            <div>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('common.date')}</p>
                        ...
                        <div className="flex flex-col items-end">
                            ...
                            <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                                {t('common.type')}: <span className="text-slate-900 dark:text-white ml-1">{appointment.type}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {/* Patient Section */}
                        <div className="space-y-4 p-6 rounded-4xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-2">
                                <UserIcon className="h-5 w-5 text-primary" />
                                <h3 className="text-lg font-black text-slate-900 dark:text-white ">{t('clinic.appointments.patient')}</h3>
                            </div>

                            <div className="flex items-center gap-4 py-2">
                                <div className="h-12 w-12 rounded-full overflow-hidden border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex items-center justify-center">
                                    {appointment.patient?.photo ? (
                                        patientPhotoUrl ? (
                                            <div className="relative h-full w-full">
                                                <UserIcon className="h-6 w-6 text-slate-400 absolute inset-0 m-auto" />
                                                <img
                                                    src={patientPhotoUrl}
                                ...
                                <div>
                                    <p className="font-bold text-slate-900 dark:text-white">{appointment.patient?.name}</p>
                                    <p className="text-sm text-slate-500">{appointment.patient?.email || t('common.noData')}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-2 gap-4 pt-2">
                                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{t('patient.profile.bloodGroup')}</p>
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        <Droplet className="h-3 w-3 text-red-500" />
                                        <span className="font-bold text-slate-900 dark:text-white">{appointment.patient?.bloodGroup || "O+"}</span>
                                    </div>
                                </div>
                                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{t('form.phone')}</p>
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        <PhoneVolumeIcon className="h-3 w-3 text-slate-400" />
                                        <span className="font-bold text-slate-900 dark:text-white break-all">{appointment.patient?.contact || t('common.noData')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 p-6 rounded-4xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-lg font-black text-slate-900 dark:text-white ">{t('clinic.appointments.doctor')}</h3>
                            </div>
                            ...
                            {!appointment.prescription ? (
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-5 bg-white/5 rounded-3xl border border-white/5 backdrop-blur-sm">
                                        <p className="text-[10px] font-black text-teal-400 uppercase tracking-widest mb-2">{t('doctor.diagnosis')}</p>
                                        <p className="text-lg font-bold leading-relaxed">{appointment.prescription.diagnosis}</p>
                                    </div>

                                    {/* Medicines Grid */}
                                    <div>
                                        <p className="text-[10px] font-black text-teal-400 uppercase tracking-widest mb-3 ml-4">{t('doctor.prescriptions.table.medicines')}</p>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="p-5 bg-white/5 rounded-3xl border border-white/5">
                                            <p className="text-[10px] font-black text-teal-400 uppercase tracking-widest mb-1">{t('doctor.notes')}</p>
                                        ...
                                        <div className="p-5 bg-white/5 rounded-3xl border border-white/5 flex flex-col justify-center">
                                            <p className="text-[10px] font-black text-teal-400 uppercase tracking-widest mb-1">{t('doctor.followUp')}</p>
                                            <div className="flex items-center gap-2">
                                    ...
                                    <div className="pt-2 flex justify-end">
                                        <div className="px-6 py-3 bg-teal-500 rounded-2xl shadow-[0_8px_30px_rgb(20,184,166,0.3)] text-right">
                                            <p className="text-[10px] font-black text-teal-100 uppercase tracking-widest leading-none">{t('doctor.profile.consultationFee')}</p>
