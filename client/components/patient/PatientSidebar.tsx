...
import { toggleSidebar, togglePatientAssistant } from "@/store/slices/uiSlice";
...
export default function PatientSidebar() {
  ...
  const navLinks = [
    ...
    { href: "/dashboard/patient/health-timeline", label: t('nav.healthTimeline'), icon: HeartPulse },
  ...
  return (
    <aside style={{ zIndex: 600 }} className={`lens-sidebar self-start hidden lg:flex flex-col h-screen overflow-hidden ${isCollapsed ? 'lens-sidebar-collapsed' : ''}`}>
      ...
      <nav className="flex flex-col gap-1 flex-1 overflow-y-auto px-3 pb-4">
        ...
        <button
          onClick={() => dispatch(togglePatientAssistant())}
          title={isCollapsed ? t('nav.patientAssistant') : ""}
          className={`${isPatientAssistantOpen ? "lens-nav-item-active" : "lens-nav-item"} ${isCollapsed ? 'justify-center px-0' : ''}`}
        >
          <Bot size={18} strokeWidth={isPatientAssistantOpen ? 2.5 : 2} className="shrink-0" />
          {!isCollapsed && <span>{t('nav.patientAssistant')}</span>}
        </button>
      </nav>
