        ...
        <div className="rounded-2xl border-l-4 border-l-primary border-y border-r border-black/5 bg-white p-6">
            ...
            <div className="mt-6 flex flex-col gap-4">
                {followUps.map((followUp: any, index: number) => {
                    const doctorName = followUp.doctorId?.doctorProfile?.fullName || followUp.doctorId?.name || "Doctor";
                    return (
                        <div
                            key={followUp._id || index}
                            className="flex flex-col gap-3 rounded-xl border border-border-light bg-background/50 p-4 transition-colors hover:bg-background dark:bg-ink-soft"
                        >
                            <div className="flex items-start justify-between">
                                <div>
                                    <p className="font-semibold text-text-primary">
                                        {t("patient.followUps.followUpFor", { diagnosis: followUp.diagnosis })}
                                    </p>
                                    <div className="mt-1 flex items-center gap-2 text-sm text-text-primary">
                                        <Stethoscope className="h-3.5 w-3.5" />
                                        <span>{t("common.doctorPrefix")} {doctorName}</span>
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center gap-4 text-xs font-medium text-text-primary">
                                {followUp.appointmentId?.clinicId && (
                                    <div className="flex items-center gap-1">
                                        <Building2 className="h-3.5 w-3.5" />
                                        <span>{followUp.appointmentId.clinicId.name}</span>
                                    </div>
                                )}
                            </div>
                        </div>
